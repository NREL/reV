...
class AbstractSamGeneration(RevPySam, ScheduledLossesMixin, ABC):
        ...
        if meta is not None:
            if sam_sys_inputs is not None:
                if MetaKeyName.ELEVATION in sam_sys_inputs:
                    meta[MetaKeyName.ELEVATION] = sam_sys_inputs[MetaKeyName.ELEVATION]
                if MetaKeyName.TIMEZONE in sam_sys_inputs:
                    meta[MetaKeyName.TIMEZONE] = int(sam_sys_inputs[MetaKeyName.TIMEZONE])
            ...
            if site_sys_inputs is not None:
                if MetaKeyName.ELEVATION in site_sys_inputs:
                    meta[MetaKeyName.ELEVATION] = site_sys_inputs[MetaKeyName.ELEVATION]
                if MetaKeyName.TIMEZONE in site_sys_inputs:
                    meta[MetaKeyName.TIMEZONE] = int(site_sys_inputs[MetaKeyName.TIMEZONE])
    ...
    def has_timezone(self):
        ...
        if self._meta is not None:
            if MetaKeyName.TIMEZONE in self.meta:
                return True
...
class AbstractSamGenerationFromWeatherFile(AbstractSamGeneration, ABC):
    ...
    WF_META_DROP_COLS = {MetaKeyName.ELEVATION, MetaKeyName.TIMEZONE, 'country', 'state', 'county',
                         'urban', 'population', 'landcover', MetaKeyName.LATITUDE,
    ...
    def set_resource_data(self, resource, meta):
        ...
        for var, arr in resource.items():
            if var != 'time_index':
                ...
                n_roll = int(self._meta[MetaKeyName.TIMEZONE] * self.time_interval)
...
class AbstractSamPv(AbstractSamSolar, ABC):
    ...
    def set_latitude_tilt_az(sam_sys_inputs, meta):
        ...
        sam_sys_inputs : dict
            ...
            present or set to 'lat' or MetaKeyName.LATITUDE, the tilt will be set to
