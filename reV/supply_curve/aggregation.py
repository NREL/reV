        ...
        if data_layers is not None:
            for name, attrs in data_layers.items():
                ...
                if 'fobj' not in attrs:
                    with Geotiff(attrs['fpath']) as f:
                        data = f[int(attrs['band']), self.rows, self.cols]
                else:
                    data = attrs['fobj'][int(attrs['band']),
                                         self.rows, self.cols]

                data = data[self.mask]
                if attrs['method'] == 'mode':
...
class Aggregation:
    ...
    def _serial_summary(fpath_excl, fpath_gen, fpath_techmap, dset_tm,
        ...
        with AggFileHandler(fpath_excl, fpath_gen, fpath_techmap,
                            data_layers) as fhandlers:

            inputs = Aggregation._get_input_data(fhandlers.gen, fpath_gen,
                                                 res_class_dset,
                                                 res_class_bins,
                                                 dset_cf,
                                                 dset_lcoe)
                ...
                for ri, res_bin in enumerate(inputs[1]):
                    try:
                        pointsum = SupplyCurvePointSummary.summary(
                            gid, fhandlers.exclusions, fhandlers.gen,
                            fhandlers.techmap,
                            dset_tm, gen_index,
                            ...
                            dset_cf=inputs[2],
                            dset_lcoe=inputs[3],
                            data_layers=fhandlers.data_layers,
