...
def test_clearsky():
    ...
    pp = ProjectPoints(slice(0, 10), sam_files, "pvwattsv5", res_file=res_file)
...
def test_proj_control_iter(start, interval):
    ...
    pp = ProjectPoints(
        slice(start, 100, interval), sam_files, "windpower", res_file=res_file
    )
...
def test_split_iter():
    ...
    pp = ProjectPoints(
        slice(0, 500, 5), sam_files, "windpower", res_file=res_file
    )
...
def test_bad_sam_configs():
    ...
    fpp = os.path.join(TESTDATADIR, "project_points/pp_offshore.csv")
    sam_files = [
        os.path.join(TESTDATADIR, "SAM/wind_gen_standard_losses_0.json"),
        os.path.join(TESTDATADIR, "SAM/wind_gen_standard_losses_1.json"),
    ]
