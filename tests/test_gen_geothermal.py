...
def sample_resource_data(request):
    ...
    with TemporaryDirectory() as td:
        ...
        Outputs.add_dataset(geo_res_file, 'temperature_2000m',
                            np.array([request.param["temp"]]),
                            np.float32, attrs={"units": "C"})
        Outputs.add_dataset(geo_res_file, 'potential_MW_2000m',
...
@pytest.mark.parametrize("sample_resource_data",
                         [{"temp": 150, "potential": 200}], indirect=True)
...
@pytest.mark.parametrize("sample_resource_data",
                         [{"temp": 60, "potential": 200}], indirect=True)
...
@pytest.mark.parametrize("sample_resource_data",
                         [{"temp": 150, "potential": 100}], indirect=True)
...
@pytest.mark.parametrize("sample_resource_data",
                         [{"temp": 150, "potential": 20}], indirect=True)
...
@pytest.mark.parametrize("sample_resource_data",
                         [{"temp": 200 * Geothermal.MAX_RT_TO_EGS_RATIO + 10,
                           "potential": 20}], indirect=True)
...
@pytest.mark.parametrize("sample_resource_data",
                         [{"temp": 200 * Geothermal.MAX_RT_TO_EGS_RATIO - 1,
                           "potential": 20}], indirect=True)
